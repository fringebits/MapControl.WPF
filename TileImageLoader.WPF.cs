...
namespace MapControl
{
    ...
    public class TileImageLoader : ITileImageLoader
    {
        ...
        public int NumPendingTiles => this.pendingTiles.Count();
        ...
        public void BeginLoadTiles(ITileLayer tileLayer, IEnumerable<Tile> tiles)
        {
            if (tiles.Any())
            {
                ...
                if (imageTileSource != null && !imageTileSource.IsAsync) // call LoadImage in UI thread with low priority
                {
                    foreach (var tile in tiles)
                    {
                        dispatcher.BeginInvoke(
                            new Action<Tile>(t => t.SetImage(LoadImage(imageTileSource, t))), 
                ...
                else
                {
                    ...
                    Task.Run(() => this.GetTiles(tileList, dispatcher, tileSource, sourceName, maxDownloads));
                }
            }
        }
        ...
        public void CancelLoadTiles(ITileLayer tileLayer)
        {
            ...
            while (this.pendingTiles.TryDequeue(out pendingTile)) ; // no Clear method
        }
